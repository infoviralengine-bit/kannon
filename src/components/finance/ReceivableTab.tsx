...
export function ReceivableTab() {
  ...
  function openEdit(p: ClientPaymentRow) {
    setEditing(p);
    setEditForm({ fixed: p.fixedAmount, cpm: p.cpmAmount, dueDate: p.dueDate, notes: p.notes ?? "" });
  }
  ...
  async function handleStatusChange(p: ClientPaymentRow, newStatus: "paid" | "invoiced" | "pending") {
    ...
    try {
      let update: any;
      if (newStatus === "paid") update = { is_paid: true, paid_at: new Date().toISOString(), invoice_sent: true };
      else if (newStatus === "invoiced") update = { is_paid: false, paid_at: null, invoice_sent: true };
      else update = { is_paid: false, paid_at: null, invoice_sent: false };
      const { error } = await supabase.from("client_payments").update(update).eq("id", p.id);
      if (error) throw error;
