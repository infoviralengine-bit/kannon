...
import {
  Check, ChevronLeft, ChevronRight, FileText, Users, ChevronDown,
} from "lucide-react";
...
export function PayableTab() {
  ...
  type CreatorBreakdown = {
    contractId: string; contractName: string; periodNumber: number;
    section: ContractPayableSection; creator: CreatorInContract;
  };
  type CreatorRollup = {
    creatorId: string; creatorName: string;
    totalPending: number; totalPaid: number; total: number;
  ...
  allSections.forEach((sec) => {
    ...
    sec.creators.forEach((cr) => {
      ...
      const cur = creatorRollupMap.get(cr.creatorId) ?? {
        creatorId: cr.creatorId, creatorName: cr.creatorName,
        totalPending: 0, totalPaid: 0, total: 0, breakdown: [],
      };
      cur.total += cr.subtotal;
      if (cr.isPaid) cur.totalPaid += cr.subtotal;
      else cur.totalPending += cr.subtotal;
      cur.breakdown.push({
        contractId: sec.contractId, contractName: sec.contractName,
        periodNumber: pn, section: sec, creator: cr,
      });
                      ...
                      <div className="flex items-center justify-between gap-3 px-4 py-3 hover:bg-background/50 transition-colors">
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <span className="font-medium text-foreground hover:underline truncate"
                            onClick={(e) => { e.stopPropagation(); navigate(`/dashboard/creators/${r.creatorId}`); }}>
                            {r.creatorName}
                          </span>
                          ...
                          <TableBody>
                            {r.breakdown.map((b) => (
                              <TableRow key={b.contractId} className="border-border hover:bg-background/50">
                                <TableCell>
                                  <span className="font-medium text-foreground hover:underline cursor-pointer text-sm"
                                ...
                                <TableCell className="text-center">
                                  <Badge className={`text-[10px] ${b.creator.fixedEarned ? "bg-emerald-500/15 text-emerald-400 border-emerald-500/30" : "bg-red-500/15 text-red-400 border-red-500/30"}`}>
                                    {formatCurrency(b.creator.fixedEarned ? b.creator.fixedAmount : 0)}
                                  </Badge>
                                </TableCell>
                                <TableCell className="text-right text-muted-foreground text-sm">{formatCurrency(b.creator.cpmAmount)}</TableCell>
                ...
                <CardHeader className="border-b border-border pb-4">
                  <div className="flex items-center justify-between flex-wrap gap-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <CardTitle className="text-base text-foreground hover:underline cursor-pointer"
                    ...
                    <TableBody>
                      {filteredCreators.map((cr) => (
                        <TableRow key={cr.creatorId} className="border-border hover:bg-background/50">
                          ...
                          <TableCell className="text-center text-muted-foreground">{cr.videoCount}/{cr.monthlyTarget}</TableCell>
                          <TableCell className="text-center">
                            <Badge className={`text-[10px] ${cr.fixedEarned ? "bg-emerald-500/15 text-emerald-400 border-emerald-500/30" : "bg-red-500/15 text-red-400 border-red-500/30"}`}>
                              {cr.fixedEarned ? "✅" : "❌"} {formatCurrency(cr.fixedEarned ? cr.fixedAmount : 0)}
                            </Badge>
                          </TableCell>
                          <TableCell className="text-right text-muted-foreground text-sm">{formatCurrency(cr.cpmAmount)}</TableCell>
