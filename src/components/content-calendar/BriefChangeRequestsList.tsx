...
export function BriefChangeRequestsList({ briefId }: { briefId: string; brief: Brief | null }) {
  ...
  const confirmAccept = async () => {
    ...
    try {
      ...
      toast({ title: t("Richiesta accettata") });
      setAccepting(null);
    } catch (e: any) {
  ...
  const reject = async (cr: BriefChangeRequest) => {
    try {
      ...
      toast({ title: t("Richiesta respinta") });
    } catch (e: any) {
      toast({ title: t("Errore"), description: e.message, variant: "destructive" });
    }
  };
  ...
  return (
    <div className="space-y-2">
      ...
      {!isLoading && crs.length === 0 && <p className="text-sm text-muted-foreground">{t("Nessuna richiesta di modifica.")}</p>}

      {crs.map((cr) => (
        <div key={cr.id} className="rounded-md border border-border p-2.5 text-sm space-y-1.5">
          <div className="flex items-center justify-between">
            <Badge className={CR_STATUS[cr.status]?.cls}>{t(CR_STATUS[cr.status]?.label ?? "")}</Badge>
            <span className="text-[10px] text-muted-foreground">{formatDateIt(cr.created_at, { day: "numeric", month: "short" })}</span>
          </div>
          <p className="text-muted-foreground"><span className="font-medium text-foreground">{t("Motivo:")}</span> {cr.reason}</p>
          {cr.proposed_copy_text != null && <p><span className="text-muted-foreground">{t("Copy:")}</span> {cr.proposed_copy_text}</p>}
          {cr.proposed_caption != null && <p><span className="text-muted-foreground">{t("Caption:")}</span> {cr.proposed_caption}</p>}
          {cr.proposed_hashtags != null && <p><span className="text-muted-foreground">{t("Hashtag:")}</span> {cr.proposed_hashtags.map((h) => `#${h}`).join(" ")}</p>}
          {cr.proposed_visual_note != null && <p><span className="text-muted-foreground">{t("Note visuali:")}</span> {cr.proposed_visual_note}</p>}
          {cr.status === "pending" && (
            <div className="flex justify-end gap-2 pt-1">
              <Button size="sm" variant="ghost" onClick={() => reject(cr)} disabled={resolve.isPending}>
                <X className="h-3.5 w-3.5 mr-1" />{t("Rifiuta")}
              </Button>
              <Button size="sm" onClick={() => { setAccepting(cr); setFields({ copy_text: true, caption: true, hashtags: true, visual_note: true }); }}>
                <Check className="h-3.5 w-3.5 mr-1" />{t("Accetta")}
              </Button>
            </div>
          )}
        </div>
      ...
      <Dialog open={!!accepting} onOpenChange={(o) => !o && setAccepting(null)}>
        <DialogContent>
          <DialogHeader><DialogTitle>{t("Applica modifiche")}</DialogTitle></DialogHeader>
          <p className="text-sm text-muted-foreground">{t("Seleziona i campi da applicare al brief.")}</p>
          <div className="space-y-2 py-2">
            {accepting?.proposed_copy_text != null && (
              <FieldRow id="copy_text" label={t("Copy")} value={accepting.proposed_copy_text} checked={fields.copy_text} onChange={(v) => setFields((f) => ({ ...f, copy_text: v }))} />
            )}
            {accepting?.proposed_caption != null && (
              <FieldRow id="caption" label={t("Caption")} value={accepting.proposed_caption} checked={fields.caption} onChange={(v) => setFields((f) => ({ ...f, caption: v }))} />
            )}
            {accepting?.proposed_hashtags != null && (
              <FieldRow id="hashtags" label={t("Hashtag")} value={accepting.proposed_hashtags.map((h) => `#${h}`).join(" ")} checked={fields.hashtags} onChange={(v) => setFields((f) => ({ ...f, hashtags: v }))} />
            )}
            {accepting?.proposed_visual_note != null && (
              <FieldRow id="visual_note" label={t("Note visuali")} value={accepting.proposed_visual_note} checked={fields.visual_note} onChange={(v) => setFields((f) => ({ ...f, visual_note: v }))} />
            )}
          </div>
          <DialogFooter>
            <Button variant="ghost" onClick={() => setAccepting(null)}>{t("Annulla")}</Button>
            <Button onClick={confirmAccept} disabled={resolve.isPending}>{t("Applica e accetta")}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
