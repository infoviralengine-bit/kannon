...
export function KPIStrip({ data }: { data: any }) {
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2">
      ...
      <Kpi icon={<Users className="h-4 w-4" />} label={t("Creator attivi")} value={String(num(d.activeCreators))}
        trend={<TrendBadge current={num(d.activeCreators)} prev={num(d.prevActiveCreators)} />} />
      <Kpi icon={<FileText className="h-4 w-4" />} label={t("Contenuti")} value={String(num(d.publishedContent))}
        trend={<TrendBadge current={num(d.publishedContent)} prev={num(d.prevPublishedContent)} />} />
      <Kpi icon={<Activity className="h-4 w-4" />} label={t("Engagement")} value={`${num(d.avgEngagementRate).toFixed(1)}%`} />
      <Kpi icon={<CalendarCheck className="h-4 w-4" />} label={t("Brief nel periodo")} value={String(num(d.briefs_count_in_period))} />
      <Kpi icon={<Trophy className="h-4 w-4" />} label={t("Brief winner")} value={String(num(d.briefs_winner_count))} />
    </div>
  );
}
