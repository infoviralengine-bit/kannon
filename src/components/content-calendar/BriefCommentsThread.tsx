  ...
  return (
    <div className="space-y-3">
      <div className="space-y-2">
        <Textarea value={body} onChange={(e) => setBody(e.target.value)} rows={2} placeholder={t("Scrivi un commento")} />
        <div className="flex justify-end">
          <Button size="sm" onClick={send} disabled={addComment.isPending || !body.trim()}>{t("Invia")}</Button>
        </div>
      </div>

      {isLoading && <p className="text-sm text-muted-foreground">{t("Caricamento...")}</p>}
      {!isLoading && comments.length === 0 && (
        <p className="text-sm text-muted-foreground">{t("Nessun commento.")}</p>
      )}
          ...
          <li key={c.id} className="rounded-md border border-border p-2.5 text-sm">
            <div className="flex items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-2">
                {c.resolved && <Badge variant="secondary" className="text-[10px]">{t("Risolto")}</Badge>}
            ...
            {staff && (
              <div className="mt-1.5 flex justify-end">
                ...
                >
                  {c.resolved ? <><Undo2 className="h-3.5 w-3.5 mr-1" />{t("Riapri")}</> : <><Check className="h-3.5 w-3.5 mr-1" />{t("Risolvi")}</>}
                </Button>
              </div>
            )}
          </li>
