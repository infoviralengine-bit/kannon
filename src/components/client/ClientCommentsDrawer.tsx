  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-full sm:max-w-md overflow-y-auto">
        <SheetHeader><SheetTitle>{t("Commenti")}</SheetTitle></SheetHeader>
        <div className="mt-4 space-y-3">
          <div className="space-y-2">
            <Textarea value={body} onChange={(e) => setBody(e.target.value)} rows={3} placeholder={t("Scrivi un commento")} />
          ...
          {!isLoading && (comments ?? []).length === 0 && <p className="text-sm text-muted-foreground">{t("Nessun commento.")}</p>}
