    ...
    <Card>
      <CardContent className="p-4 space-y-3">
        <div className="flex items-center justify-between gap-2">
          <Badge className={`${meta.badge} text-sm px-3 py-1`}>{t(meta.label)}</Badge>
        ...
        <div className="space-y-1">
          <h3 className="text-lg font-semibold leading-tight">{brief.title || t("Brief")}</h3>
          {brief.format_name && <Badge variant="secondary">{brief.format_name}</Badge>}
        </div>

        {brief.reference_links.length > 0 && (
          <div className="space-y-1">
            <p className="text-[11px] font-medium uppercase tracking-wide text-muted-foreground/70">{t("Riferimenti")}</p>
        ...
        <CopyableField label={t("Copy (testo da dire)")} value={brief.copy_text} copyLabel={t("Copy copiato")} />
        {brief.caption && <CopyableField label={t("Caption")} value={brief.caption} copyLabel={t("Caption copiata")} />}
        {brief.hashtags.length > 0 && (
          <CopyableField label={t("Hashtag")} value={brief.hashtags.map((h) => `#${h}`).join(" ")} copyLabel={t("Hashtag copiati")} />
        )}
        {brief.visual_note && (
          <div className="space-y-1">
            <p className="text-[11px] font-medium uppercase tracking-wide text-muted-foreground/70">{t("Note visuali")}</p>
