        ...
        <Card key={c.id} className="cursor-pointer hover:border-primary/50"
          ...
          <CardContent className="flex flex-wrap items-center justify-between gap-3 p-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Dal {formatDateIt(c.start_date)}</p>
            </div>
            <div className="flex items-center gap-3 text-xs text-muted-foreground">
              {c.client_fixed != null && <span>Fisso {formatCurrency(Number(c.client_fixed))}</span>}
              {c.client_cpm != null && <span>CPM {formatCurrency(Number(c.client_cpm))}</span>}
              <Badge variant="outline" className="text-[10px]">{c.status}</Badge>
            </div>
          </CardContent>
        </Card>
      ))}
      {!campaigns.length && <p className="text-sm text-muted-foreground">Nessuna campagna collegata.</p>}
    </div>
  );
}
