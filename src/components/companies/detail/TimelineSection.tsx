...
export function TimelineSection({ activities, documents, tasks, notes, authorName }: {
  ...
  const items = useMemo<Item[]>(() => {
    ...
    activities.forEach((a) => {
      ...
      const extra = [a.participants && `Partecipanti: ${a.participants}`, a.outcome && `Esito: ${a.outcome}`,
        a.objections && `Obiezioni: ${a.objections}`, a.next_steps && `Prossimi passi: ${a.next_steps}`, a.full_text]
        .filter(Boolean).join("\n");
      const text = a.summary ?? a.body ?? "";
      out.push({
        id: `a-${a.id}`, kind, date: a.occurred_at,
        title: a.type === "cambio_stadio" ? prettyStage(text) : text,
        label: ACTIVITY_TYPE_LABEL[a.type as ActivityType] ?? a.type,
    ...
    documents.forEach((d) => out.push({
      ...
      label: d.direction === "da_inviare" ? "File inviato" : "File ricevuto",
    }));
    tasks.filter((t) => t.is_done && t.done_at).forEach((t) => out.push({
      id: `t-${t.id}`, kind: "task", date: t.done_at!, title: t.title, label: "Task completato", detail: t.notes,
    }));
    notes.forEach((n) => out.push({
      id: `n-${n.id}`, kind: "nota", date: n.created_at, title: n.body.split("\n")[0].slice(0, 140),
      label: "Appunto", detail: n.body.length > 140 || n.body.includes("\n") ? n.body : null, author: authorName(n.author_id),
    }));
  ...
  return (
    <div className="space-y-3">
      ...
      <ol className="relative ml-3 space-y-3 border-l border-border/60 pl-5">
          ...
          return (
            <li key={i.id} className="relative">
              ...
              <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                ...
                {i.direction && <span>{ACTIVITY_DIRECTION_LABEL[i.direction as ActivityDirection] ?? i.direction}</span>}
        ...
        {!visible.length && <p className="text-sm text-muted-foreground">Nessun evento.</p>}
      </ol>
    </div>
  );
}
